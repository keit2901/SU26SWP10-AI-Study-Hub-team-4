...
public sealed class RagSearchService : IRagSearchService
{
    ...
    {
        query = query.Where(c =>
            (c.Document.UserId == userId || (c.Document.Folder != null
                && c.Document.Folder.ShareStatus == FolderStatus.Approved
                && c.Document.ReviewStatus == DocumentReviewStatus.Approved)) &&
